/*******************************************************************************
 * $Revision:: 144 $: Revision of last commit $LastChangedBy:: $: Author of last
 * commit $LastChangedDate:: 2011-10-27 10:08:07#$: Date of last commit
 ******************************************************************************/
/*
 * charity.js
 * 
 ...
 */
/*
 * api references: http://www.datatables.net/api
 * 
 */
...
function downloadPDF(loginId, jobId) {
	...
	requestXMLData(refreshCacheRequest, url, function() {
		// alert("ccJobCost - downloadPDF refreshCacheRequest.readyState " +
		// refreshCacheRequest.readyState);
		if (refreshCacheRequest.readyState == 4) {
			// alert("ccJobCost - downloadPDF refreshCacheRequest.status " +
			// refreshCacheRequest.status);
			if (refreshCacheRequest.status == 200) {
				// alert("ccJobCost - downloadPDF loginId " + loginId + " jobId
				// " + jobId);
				var url = 'http://localhost/ccJobCost2FPDF.php?account='
						+ loginId + '&jobId=' + jobId;
	...
	// var url = 'http://localhost/ccJobCost2FPDF.php?account=' + loginId +
	// '&jobId=' + jobId;
	// window.location.href = url;
}
...
function requestXMLData(myRequest, url, responseHandler) {
	...
	myRequest.onreadystatechange = responseHandler;
	// send the request
	// alert("sending ajax request using url " + url);
	// e.g. sending ajax request using url
	// donations.php?account=0&amount=0&charityId=8&remove=false&id=0
	myRequest.send(null);
}
...
// columns to show
// charityName shortName averageDonation numberZeroAmounts numberNonZeroAmounts
// lastAmount ACTIONS
...
function refreshCharities(torfLoggedIn, torfDetail) {
	...
	// alert("charity refreshCharities torfLoggedIn " + torfLoggedIn + "
	// loggedIn " + loggedIn + " torfDetail " + torfDetail + " detail " +
	// detail);
			...
			function() {
				if (charityRequest.readyState == 4) {
					...
					if (charityRequest.status == 200) {
						...
						// Name</th><th>Short
						// Name</th><th>Solicitations</th><th>Donations</th><th>Average</th><th>Amount</th><th>Action(<input
						// type="image" onclick="addCharity()"
						// src="images/icon_plus.gif"';
						var html;
						if (detail == true) {
							// tjs110308
							// html = '<table
							// id="charityDetailTable"><thead><tr><th>ID</th><th>Charity
							// Name</th><th>Short
							// Name</th><th>Dunns</th><th>URL</th><th>Description</th><th>Rating</th><th>Action(<input
							// type="image" onclick="addCharityDetail()"
							// src="images/icon_plus.gif"';
								...
								.each(
										function() {
											...
											if (detail) {
												...
												/*
												 * var url = ''; if
												 * (children[3].firstChild) {
												 * url =
												 * children[3].firstChild.nodeValue;
												 * url = removeSpaces(url); html += '<td><a
												 * href="http://' + url + '" >' +
												 * name + '</a></td>'; } else {
												 * html += '<td>' + name+ '</td>'; }
												...
												// html += '<td><input
												// type="image"
												// onclick="doAmountTransaction(0,'
												// + charityId + ',' +
												// loginAccountNumber + ', 0,
												// 0)"
												// src="images/icon_plus.gif"/>
												// <input type="image"
												// onclick="modifyCharity(' +
												// charityId + ',\'' + name +
												// '\',\'' + shortName + '\')"
												// src="images/edit.gif"/>&nbsp;<input
												// type="image"
												// onclick="removeCharity(' +
												// charityId + ')"
												// src="images/delete.gif"/>&nbsp;<input
												// type="image"
												// onclick="populateDonations('
												// + loginAccountNumber + ',' +
												// charityId + ')"
												// src="images/magnifier.png"/></td>';
												html += '<td><input type="image" onclick="doAmountTransaction(0,'
														+ charityId
														+ ','
														//+ loginAccountNumber
														+ memberId
														+ ', 0, 0)" src="images/icon_plus.gif"';
												if (loggedIn == false) {
													html += ' disabled="disabled" ';
												}
												...
												html += '/>&nbsp;<input type="image" onclick="removeCharity('
														+ charityId
														+ ')" src="images/delete.gif"';
												if (loggedIn == false) {
													html += ' disabled="disabled" ';
												}
												html += '/>&nbsp;<input type="image" onclick="populateDonations('
													//+ loginAccountNumber
														+ memberId
														+ ','
														+ charityId
						...
						if (detail == true) {
							...
							jQuery('#charityTable').dataTable({
								 ...
								 * iEnd, iMax, iTotal, sPre ) { //return iStart +"
								 * to "+ iEnd; if (iMax != iTotal) { if
...
function doAmountTransaction(trxType, charityId, memberId, id, newAmount,
		newDate) {
	//alert ("charity doAmountTransaction charity id " + charityId + " member id " + memberId + " id " + id + " new amount " + newAmount + " new date " + newDate);
	// e.g. charity doAmountTransaction charity id 8 member id 0 id 0 new amount
	// 0 new date undefined
// e.g. charity doAmountTransaction charity id 7 member id 1 id 2 new amount 20 new date 2013-09-02
	...
	// tjs 130726
	var url = 'donations.php?account=' + memberId + '&amount=' + amount
			+ '&charityId=' + charityId + '&remove=' + remove + '&id=' + id;
	// var url = './donations.php?account=' + memberId + '&amount=' + amount +
	// '&charityId=' + charityId + '&remove=' + remove + '&id=' + id;
	// temp hack
	// var url = 'http://localhost:8981/donations.php?account=' + memberId +
	// '&amount=' + amount + '&charityId=' + charityId + '&remove=' + remove +
	// '&id=' + id;
	...
	// alert("charity doAmountTransaction url " + url);
	// e.g. charity doAmountTransaction url
	// donations.php?account=0&amount=40&charityId=8&remove=false&id=0

	requestXMLData(donationsRequest, url, function() {
		if (donationsRequest.readyState == 4) {
			// if server HTTP response is "OK"
			// alert("charity doAmountTransaction readyState 4
			// customerRequest.status " + customerRequest.status);
		...
		// tjs 130726
			...
			// issue an error message for any other HTTP response
			// alert("An error has occurred not ready!");
		}
	});
}

/*
 * <donations> <donation id="10"> <memberId>0</memberId> <charityId>4</charityId>
 * <amount>444</amount> <date>2010-11-19 16:06:45</date> </donation>
 * </donations>
 */
function populateDonations(memberId, charityId) {
	//alert ("charity populateDonations charity id " + charityId + " member id " + memberId);
	// e.g. charity populateDonations charity id 7 member id 1
	var html;
	$('#donationList').empty();
	if (charityId == 0) {
		lastDonationCharityId = 0;
		// html = '<table
		// id="donationTable"><thead><tr><th>ID</th><th>Amount</th><th>Date</th><th>Action</th></tr></thead><tbody
		// /></table>';
		...
		var url = 'getDonationsXML.php?account=' + memberId + '&charityId='
				+ charityId;
		// alert("charity refreshCharities url " + url);
		requestXMLData(
				donationRequest,
				...
				function() {
					if (donationRequest.readyState == 4) {
						...
						if (donationRequest.status == 200) {
							...
							// var html = '<table
							// id="charityTable"><thead><tr><th>ID</th><th>Charity
							// Name</th><th>Short Name</th><th>Action(<input
							// type="image" onclick="add(4,' +
							// loginAccountNumber + ')"
							// src="images/icon_plus.gif"/>)</th></tr></thead><tbody>';
							// var html = '<table
							// id="donationTable"><thead><tr><th>ID</th><th>Amount</th><th>Date</th><th>Action(<input
							// type="image" onclick="add(4,' +
							// loginAccountNumber + ')"
							// src="images/icon_plus.gif"/>)</th></tr></thead><tbody>';
							// html = '<table
							...
							$(data)
									...
									.each(
											function() {
												...
												// '" disabled="disabled"
												// />&nbsp;&nbsp;</td>';
												html += '<td><input type="image" onclick="modifyDonation('
														+ id
														+ ','
														+ memberId
														+ ','
														...
														+ ',\''
														+ date
														+ '\')" src="images/edit.gif"/>&nbsp;<input type="image" onclick="removeDonation('
														+ id
														+ ','
														+ memberId
														+ ','
							...
							// issue an error message for any other HTTP
							// response
							alert("An error has occurred: "
									+ donationRequest.statusText);
						}
					}
				});
	}
...
// function modifyCharityDetail(id, name, shortName, dunns, url, description,
// numStars) {
...
// function removeCharityDetail(id, name, shortName, dunns, url, description,
// numStars) {
function removeCharityDetail(id, name, shortName, dunns, isForProfit,
		isInactive, url, description, numStars) {
	document.charityDetailForm.id.value = id;
...
function modifyDonation(id, memberId, charityId, amount, date) {
	...
	// $("#datepicker").datepicker({dateFormat: 'yy-mm-dd', minDate: '-120M',
	// maxDate: 1, defaultDate: date });
	...
	// e.g. charity modifyDonation amount 10 date 2013-09-02 00:00:00 date1 2013-09-02
	//alert("charity modifyDonation amount " + amount + " date " + date + " date1 " + date1 + " id " + id + " memberId " + memberId + " charityId " + charityId);
	// e.g. charity modifyDonation amount 10 date 2013-09-02 00:00:00 date1 2013-09-02 id 2 memberId 1 charityId 7
	
	var date0 = new Date(date1);
	// alert("charity modifyDonation amount " + amount + " date " + date + "
	// date0 " + date0);
	...
	// alert("charity modifyDonation amount " + amount + " date2 " + date2 + "
	// madeOn " + madeOn);
	// charity modifyDonation amount 0 date 2010-11-10 00:00:00 madeOn
	// 2010-12-04
...
function removeDonation(id, memberId, charityId, amount, date) {
	...
	// queryStr = "input#" + id;
	// var name =
	// jQuery(queryStr).parent().parent().children()[1].firstChild.nodeValue;
	document.donationForm.amount.value = amount;
	// alert("charity removeCharity name " + name);
	// var shortname =
	// jQuery(queryStr).parent().parent().children()[2].firstChild.nodeValue;
	// document.donationForm.date.value = date;
	// alert("charity removeCharity shortname " + shortname);
	// $("#datepicker").datepicker({dateFormat: 'yy-mm-dd', minDate: '-120M',
	// maxDate: 1, defaultDate: date });
...
function processCharityDetailForm() {
	...
	// description + '&numStars=' + numStars + '&id=' + id + '&remove=' +
	// remove;
	url = 'charitiesDetail.php?charityName=' + name + '&shortName=' + shortname
			+ '&dunns=' + dunns + '&isForProfit=' + isForProfit
...
function processDonationForm() {
	...
	// e.g. charity processDonationForm amount 20 madeOn 2013-09-02
	...
	if (remove == 'true') {
		...
		// alert ("charity processDonationForm blank " +
		// document.donationForm.blank.checked + " currency " +
		...
		// postRatingsUpdate(charityId, memberId, id, madeOn, blank, currency,
		// reminder, confidential);
		// tjs 130314
		postRatingsUpdate(id, charityId, memberId, madeOn, blank, currency,
				reminder, confidential);
		// postRatingsUpdate(id, charityId, memberId, year, blank, currency,
		// reminder, confidential);
		// tjs 120321
...
// tjs 130314
function postRatingsUpdate(donationId, charityId, memberId, date, blank,
		currency, reminder, confidential) {
	// function postRatingsUpdate(donationId, charityId, memberId, year, blank,
	// currency, reminder, confidential) {
	...
	var url = 'ratings.php?account=' + memberId + '&charityId=' + charityId
...
// tjs 130226
function postAggregateRatingsUpdate(donationId, charityId, memberId, date,
		blank, currency, reminder, confidential) {
	...
	// tjs110308
	// var url = 'getCharitiesXML.php?account=' + loginAccountNumber;
	var url = 'getBaseCharityXML.php?account=' + loginAccountNumber
			+ '&memberCharityId=' + charityId;
	// alert("charity postAggregateRatingsUpdate url " + url);
	// alert("charity postAggregateRatingsUpdate loggedIn " + loggedIn + "
	...
	postRequestXMLData(
			...
			function() {
				if (charityRequest.readyState == 4) {
					// if server HTTP response is "OK"
					// alert("charity postAggregateRatingsUpdate readyState 4
					// charityRequest.status " + charityRequest.status);
					if (charityRequest.status == 200) {
						var data = charityRequest.responseXML;
						var provider = $(data).find('charities').attr(
								'provider');
						var database = $(data).find('charities').attr(
								'database');
						$(data)
								.find('charity')
								.each(
										function() {
											...
											if (charityId != memberCharityId) {
												...
												if (blank) {
													...
													// tjs 130314
													syncAggregateRatingsUpdate(
															provider, database,
															donationId,
															charityId, name,
															year, aggregateList);
													// syncAggregateRatingsUpdate(provider,
													// database, donationId,
													// charityId, name, date,
													// aggregateList);
												}
						...
						// issue an error message for any other HTTP response
						alert("postAggregateRatingsUpdate An error has occurred: "
								+ ratingsRequest.statusText);
					}
				}
			});
...
// tjs 130314 - remove support for donation lists, add support to bump year's
// score by one
function syncAggregateRatingsUpdate(provider, database, donationId, charityId,
		name, year, aggregateList) {
	// alert("syncAggregateRatingsUpdate provider " + provider + " database " +
	// database + " charityId " + charityId + " year " + year + " list " +
	// aggregateList);
	// e.g. syncAggregateRatingsUpdate provider firebaseIO.com database
	// collogistics charityId 9 year 2013 list blankScoreList
	...
	// e.g. syncAggregateRatingsUpdate url
		...
		if (currentScore == 1) {
				...
				yearScoreRef.set(currentScore, function(error) {
					if (error == null) {
						...
						userScoreRef.once('value', function(dataSnapshot) {
							// Given a DataSnapshot containing a child 'fred'
							// and a child 'wilma':
							var cumYearScores = 0;
							dataSnapshot.forEach(function(childSnapshot) {
								...
								if (name != 'name') {
									...
									// alert("bumpScore for cum childData " +
									// childData);
									cumYearScores += Number(childData);
								}
								// name will be 'fred' the first time and
								// 'wilma' the second time.
								// childData will be the actual contents of the
								// child.
							});
							userScoreRef.setPriority(cumYearScores, function(
									error) {
								// alert("bumpScore cumYearScores " +
								// cumYearScores);
							});
						});
					} else {
			...
			yearScoreRef.set(currentScore, function(error) {
				if (error == null) {
					// userScoreRef.on('value', function(dataSnapshot) {
					userScoreRef.once('value', function(dataSnapshot) {
						// Given a DataSnapshot containing a child 'fred' and a
						// child 'wilma':
						var cumYearScores = 0;
						dataSnapshot.forEach(function(childSnapshot) {
							// This code will be called twice.
							...
							// name will be 'fred' the first time and 'wilma'
							// the second time.
							// childData will be the actual contents of the
							// child.
						});
						userScoreRef.setPriority(cumYearScores,
								function(error) {
									// alert("bumpScore cumYearScores " +
									// cumYearScores);
								});
